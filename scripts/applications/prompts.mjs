...
6. Write in the requested language only, formal register, professional and concrete. Short sentences.
   FORBIDDEN WORDS in any language (the page is rejected if one appears): ${FORBIDDEN_WORDS}. Talk about precise measuring, less waste, storage space and logistics instead of money, and about "preparation according to the technical sheet" instead of describing how the powder is prepared.
...
const SCHEMA = (app) => `{
  "seo": { "description": "meta description: exactly two full sentences, the first states the choice (liquid or powder for this application) with the keyword, the second says who it is for (chefs, bakeries, bars, industry); no digits" },
  "answer": "one or two sentences that complete the answer to 'liquid or powder?' for this application, qualitatively, no digits",
  "sections": [
...
- Only claims from this list: ${ALLOWED_CLAIMS}
- Nothing about how the powder is prepared, nothing about prices or costs, no shelf-life comparison, no invented specifications, no health claims, no guarantees.
...
export function buildAuditPrompt({ locale, app, entry, facts, keyword }) {
