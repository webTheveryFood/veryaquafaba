...
import { APPLICATION_ROOTS, APPLICATION_ALIASES, CHILD_ALIASES, RESOURCES_ROOTS, TOOLS_ROOTS, SECTION_ROOTS, TOPIC_ALIASES } from '../data/applications/routes';

const RESOURCES_ROUTES = new Set([...Object.values(RESOURCES_ROOTS), ...Object.values(TOOLS_ROOTS)]);
...
function typeForRoute(route, locale) {
  ...
  if (RESOURCES_ROUTES.has(route)) return 'resources';
  const appRoot = APPLICATION_ROOTS[locale];
  // /resources/applications/ is the index; one segment below it a guide; two, a child
  // (quantity calculator, process sheet) of that guide.
  if (appRoot && route === appRoot) return 'application-index';
  if (appRoot && route.startsWith(appRoot)) return route.slice(appRoot.length).split('/').filter(Boolean).length > 1 ? 'application-child' : 'application';
  // Set-2 sections (professionals, reference, egg substitutes, where to buy): the root is
  // the section's pillar page, one segment below it a topic page.
  for (const roots of Object.values(SECTION_ROOTS)) {
    const root = roots[locale];
    if (root && route === root) return 'topic-index';
    if (root && route.startsWith(root)) return 'topic';
  }
...
function translationKey(route, type) {
  ...
  if (type === 'resources') return Object.values(TOOLS_ROOTS).includes(route) ? 'resources:tools' : 'resources';
  if (type === 'legal') return legalTranslationKey(route);
  if (type === 'utility') return utilityTranslationKey(route);
  // Application guides: localized slug (baiser) -> canonical key (meringue).
  if (type === 'application') {
    const slug = slugForRoute(route);
    return `application:${APPLICATION_ALIASES[slug] || slug}`;
  }
  if (type === 'application-index') return 'application-index';
  if (type === 'topic-index' || type === 'topic') {
    const locale = localeForRoute(route);
    const section = Object.keys(SECTION_ROOTS).find((s) => route.startsWith(SECTION_ROOTS[s][locale]));
    if (type === 'topic-index') return `topic-index:${section}`;
    const slug = slugForRoute(route);
    // Where-to-buy leaves are countries, grouped by country (data/resources/stockists.js sets the slug).
    return `topic:${section}:${TOPIC_ALIASES[section]?.[slug] || slug}`;
  }
  if (type === 'application-child') {
    const [app, child] = route.split('/').filter(Boolean).slice(-2);
    return `application-child:${APPLICATION_ALIASES[app] || app}:${CHILD_ALIASES[child] || child}`;
  }
...
export const PROGRAMMATIC_PAGE_TYPES = ['home', 'about', 'buy', 'recipe-index', 'recipe', 'application', 'application-index', 'application-child', 'topic-index', 'topic', 'resources'];
...
export const pageRegistry = allRoutes.map((route) => {
  ...
  return {
    ...
    isProgrammatic: PROGRAMMATIC_PAGE_TYPES.includes(type),
  };
});
